...
)
{
    ...
    confirmation_data_response_local_var->edhoc_payload = edhoc_payload;
...
void OpenAPI_confirmation_data_response_free(OpenAPI_confirmation_data_response_t *confirmation_data_response)
{
    ...
    if (confirmation_data_response->edhoc_payload) {
        ogs_free(confirmation_data_response->edhoc_payload);
        confirmation_data_response->edhoc_payload = NULL;
    }
...
cJSON *OpenAPI_confirmation_data_response_convertToJSON(OpenAPI_confirmation_data_response_t *confirmation_data_response)
{
    ...
    if (confirmation_data_response->edhoc_payload) {
    if (cJSON_AddStringToObject(item, "edhocPayload", confirmation_data_response->edhoc_payload) == NULL) {
        ogs_error("OpenAPI_confirmation_data_response_convertToJSON() failed [edhoc_payload]");
        goto end;
    }
    }
...
OpenAPI_confirmation_data_response_t *OpenAPI_confirmation_data_response_parseFromJSON(cJSON *confirmation_data_responseJSON)
{
    ...
    edhoc_payload = cJSON_GetObjectItemCaseSensitive(confirmation_data_responseJSON, "edhocPayload");
    if (edhoc_payload) {
    if (!cJSON_IsString(edhoc_payload) && !cJSON_IsNull(edhoc_payload)) {
        ogs_error("OpenAPI_confirmation_data_response_parseFromJSON() failed [edhoc_payload]");
        goto end;
    }
    }
